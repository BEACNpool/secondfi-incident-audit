...
| `evidence/lane_b/` | Key-exposure detector validation + scoped-census bundle: raw tx CBOR inputs, detector output, and the public per-ring census aggregate (`census_summary.csv`; 2,588 stakes cryptographically confirmed exposed). Exposure-only, no key recovery; the row-level exposed-wallet list is withheld. |
